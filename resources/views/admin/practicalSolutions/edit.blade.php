...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.practical-solutions.update", [$practicalSolution->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.practicalSolution.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $practicalSolution->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description" required>{{ old('description', $practicalSolution->description) }}</textarea>
...
<script>
    ...
    url: '{{ route('admin.practical-solutions.storeMedia') }}',
    maxFilesize: 2, // MB
