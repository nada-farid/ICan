...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.videos-participates.update", [$videosParticipate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="title">{{ trans('cruds.videosParticipate.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $videosParticipate->title) }}" required>
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $videosParticipate->description) }}</textarea>
            ...
            <div class="form-group">
                <label>{{ trans('cruds.videosParticipate.fields.status') }}</label>
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    <option value disabled {{ old('status', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\VideosParticipate::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $videosParticipate->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.videosParticipate.fields.status_helper') }}</span>
            </div>
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\VideosParticipate::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $videosParticipate->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('embed_code') ? 'is-invalid' : '' }}" name="embed_code" id="embed_code">{{ old('embed_code', $videosParticipate->embed_code) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('practical_solution') ? 'is-invalid' : '' }}" name="practical_solution_id" id="practical_solution_id">
                    @foreach($practical_solutions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('practical_solution_id') ? old('practical_solution_id') : $videosParticipate->practical_solution->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('champion') ? 'is-invalid' : '' }}" name="champion_id" id="champion_id">
                    @foreach($champions as $id => $entry)
                        <option value="{{ $id }}" {{ (old('champion_id') ? old('champion_id') : $videosParticipate->champion->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
