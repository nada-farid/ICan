...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.publicSubject.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.public-subjects.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subject_category') ? 'is-invalid' : '' }}" name="subject_category_id" id="subject_category_id" required>
                    @foreach($subject_categories as $id => $entry)
                        <option value="{{ $id }}" {{ old('subject_category_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
